...
class tabla{
	...
	public function __construct($programa){
		$this->TablaBD = "resultados"; //Tabla a la que se se le va a hacer el CRUD
		$this->TablaVisual = "Resultados de Aprendizaje de $programa"; //Cómo se va a ver en la parte visual
	...
	public function Busqueda($nombre, $programa, $Posicion){
		$SQL = "SELECT codigo, nombre FROM $this->TablaBD WHERE nombre LIKE :nombre AND programa = :programa LIMIT $Posicion, 1";
	...
	public function Adicionar($nombre, $programa)
	{
		$SQL = "INSERT INTO $this->TablaBD (nombre, programa) VALUES(:nombre, :programa)";
	...
	{
		$SQL = "SELECT codigo, nombre FROM $this->TablaBD WHERE programa = :programa ORDER BY nombre";
		$Sentencia = $this->BaseDatos->Conexion->prepare($SQL);
		$Sentencia->bindValue(":programa", $programa);
		$Sentencia->execute();  //Ejecuta la consulta
