...
switch(abs(intval($_GET["op"]))) {
	case 0: //Inicia actualización
		...
		$Pantalla = str_replace("{identifica}", $Registros[1], $Pantalla);
		$Pantalla = str_replace("{cboPrograma}", $Tabla->ComboBoxPrograma($Registros[2]), $Pantalla);
		$Pantalla = str_replace("{nombre}", $Registros[3], $Pantalla);
		$Pantalla = str_replace("{correo1}", $Registros[4], $Pantalla);
		$Pantalla = str_replace("{correo2}", $Registros[5], $Pantalla);
		break;
	case 1: //Inicia borrado
		...
		$Pantalla = str_replace("{codigo}", $Registros[0], $Pantalla);
		$Pantalla = str_replace("{identifica}", $Registros[1], $Pantalla);
		$Pantalla = str_replace("{usuarioprograma}", $Registros[2], $Pantalla);
		$Pantalla = str_replace("{nombre}", $Registros[3], $Pantalla);
		$Pantalla = str_replace("{correo1}", $Registros[4], $Pantalla);
		$Pantalla = str_replace("{correo2}", $Registros[5], $Pantalla);
		break;
